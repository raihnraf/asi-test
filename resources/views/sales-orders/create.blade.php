    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('sales-orders.store') }}" class="space-y-6 p-6">
                    @csrf

                    @include('sales-orders.partials.form', ['salesOrder' => null])
                    ...
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <x-primary-button>{{ __('Save Sales Order') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
